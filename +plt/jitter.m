...
if createOpt || ~isfield(opt, 'centerOperation'); opt.centerOperation = @(x) mean(x, 'omitnan'); end
if createOpt || ~isfield(opt, 'errorOperation'); opt.errorOperation = @(x) std(x, 'omitnan'); end
