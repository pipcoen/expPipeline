...
if ~exist('trialGroups','var') || isempty(trialGroups)
   trialGroups =  cellfun(@(x) ones(size(x,1),1),eventTimes,'uni',false);
end
...
ephWaveforms = blk.ephWaveforms;
channelPositions = blk.ephChannelMap;
clusterDepths = blk.ephClusterDepths;
spikeTimes = blk.ephSpikeTimes;
ephClusterID = blk.ephClusterID;
% clusterDepths = clusterDepths(unique(ephClusterID));
spikeAmps = blk.ephSpikeAmps;

% Sort the units by depth if not specified
if ~exist('sortRule','var')
   [~,sortRule] = sort(clusterDepths); 
end
...
% (user inputs)
gui_data.eventTimes = eventTimes;
gui_data.trialGroups = trialGroups;
gui_data.sortRule = sortRule;

% (spike data)
gui_data.ephWaveforms = ephWaveforms;
gui_data.channelPositions = channelPositions;
gui_data.spikeTimes = spikeTimes;
gui_data.ephClusterID = ephClusterID;
gui_data.spikeAmps = spikeAmps;

% (current settings)
gui_data.curr_unit = sortRule(1);
...
template_y = permute(mean(gui_data.ephWaveforms(gui_data.curr_unit,:,:),1),[3,2,1]);
template_y = -template_y*template_yscale + gui_data.channelPositions(:,2);
template_x = (1:size(gui_data.ephWaveforms,2)) + gui_data.channelPositions(:,1)*template_xscale;

template_channel_amp = range(gui_data.ephWaveforms(gui_data.curr_unit,:,:),2);
template_thresh = max(template_channel_amp,[],3)*0.2;
template_use_channels = any(template_channel_amp > template_thresh,1);
[~,max_channel] = max(max(abs(gui_data.ephWaveforms(gui_data.curr_unit,:,:)),[],2),[],3);

arrayfun(@(ch) set(gui_data.waveform_lines(ch),'XData',template_x(ch,:),'YData',template_y(ch,:)),1:size(gui_data.ephWaveforms,3));
...
curr_spikes_idx = ismember(gui_data.ephClusterID,gui_data.curr_unit);
curr_raster_spikeTimes = gui_data.spikeTimes(curr_spikes_idx);
curr_raster_spikeTimes(curr_raster_spikeTimes < min(gui_data.t_peri_event(:)) | ...
    curr_raster_spikeTimes > max(gui_data.t_peri_event(:))) = [];

curr_raster = cell2mat(arrayfun(@(x) ...
    histcounts(curr_raster_spikeTimes,gui_data.t_peri_event(x,:)), ...
    [1:size(gui_data.t_peri_event,1)]','uni',false));

% Set color scheme
curr_group = gui_data.trialGroups{gui_data.curr_align}(:,gui_data.curr_group);
...
if length(gui_data.curr_unit) == 1
    ...
    [~,~,row_group] = unique(gui_data.trialGroups{gui_data.curr_align}(:,gui_data.curr_group),'sorted');
...
if length(gui_data.curr_unit) == 1
    set(gui_data.amplitude_plot,'XData', ...
        gui_data.spikeTimes(curr_spikes_idx), ...
        'YData',gui_data.spikeAmps(curr_spikes_idx),'linestyle','none');
elseif length(gui_data.curr_unit) > 1
    long_bin_size = 60;
    long_bins = gui_data.spikeTimes(1):long_bin_size:gui_data.spikeTimes(end);
    long_bins_t = long_bins(1:end-1) + diff(long_bins)/2;
    long_spikes_binned = discretize(gui_data.spikeTimes,long_bins);
    amplitude_binned = accumarray(long_spikes_binned(curr_spikes_idx & ~isnan(long_spikes_binned)), ...
        gui_data.spikeAmps(curr_spikes_idx & ~isnan(long_spikes_binned)),size(long_bins_t'),@nansum,NaN);
...
switch eventdata.Key
    case 'downarrow'
        % Next unit
        curr_unit_idx = gui_data.curr_unit(1) == gui_data.sortRule;
        new_unit = gui_data.sortRule(circshift(curr_unit_idx,1));
        gui_data.curr_unit = new_unit;
        
    case 'uparrow'
        % Previous unit
        curr_unit_idx = gui_data.curr_unit(end) == gui_data.sortRule;
        new_unit = gui_data.sortRule(circshift(curr_unit_idx,-1));
        gui_data.curr_unit = new_unit;
    ...
    case 'rightarrow'
        ...
        if new_align > length(gui_data.eventTimes)
            new_align = 1;
        end
        use_align = reshape(gui_data.eventTimes{new_align},[],1);
    ...
    case 'leftarrow'
        ...
        new_align = gui_data.curr_align - 1;
        if new_align < 1
            new_align = length(gui_data.eventTimes);
        end
        use_align = reshape(gui_data.eventTimes{new_align},[],1);
        t_peri_event = use_align + gui_data.t_bins;
    ...
    case 'pagedown'
        ...
        if next_group > size(gui_data.trialGroups{gui_data.curr_align},2)
            next_group = 1;
        end
    ...
    case 'pageup'
        ...
        next_group = gui_data.curr_group - 1;
        if next_group < 1
            next_group = size(gui_data.trialGroups{gui_data.curr_align},2);
        end
        gui_data.curr_group = next_group;
    ...
    case 'm'
        ...
        clusterDepths = get(gui_data.unit_dots,'YData');
        gui_data.curr_unit = find(clusterDepths >= multiunit_top & ...
            clusterDepths <= multiunit_bottom);       
        
    case 'u'
        % Enter and go to unit
        new_unit = str2num(cell2mat(inputdlg('Go to unit:')));
        if ~ismember(new_unit,unique(gui_data.ephClusterID))
