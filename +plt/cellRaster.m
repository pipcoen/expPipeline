...
for i = find(cellfun(@(x) size(x,2)==1, eventTimes))'
    if length(eventTimes{i}) == length(blk.sessionIdx); eventTimes{i} = [eventTimes{i} blk.sessionIdx];
...
numSessions = unique(guiData.blk.sessionIdx);
numSites = cellfun(@length, guiData.blk.ephClusterTemplates);
guiData.sessionsAndSites = cell2mat(arrayfun(@(x,y) [x*ones(y,1) (1:y)'], numSessions, numSites, 'uni', 0));

guiData.currSession = guiData.sessionsAndSites(1,1);
guiData.currSite = guiData.sessionsAndSites(1,2);
...
switch guiData.sortRule(1:3)
    ...
    case 'sig'
        ...
        blk = prc.filtStruct(blk, blk.sessionIdx == guiData.currSession);
        blk.ephSpikeCluster = blk.ephSpikeCluster-min(blk.ephSpikeCluster)+1;
        guiData.clusterSigLevel = cellfun(@(x) kil.findResponsiveCells(blk,x), guiData.currEventTimes, 'uni', 0);
...
switch eventdata.Key
    case {'equal';'hyphen';'s'}
        ...
        guiData.sessionsAndSites = circshift(guiData.sessionsAndSites, [-moveDir, 0]);
        guiData.currSession = guiData.sessionsAndSites(1,1);
        guiData.currSite = guiData.sessionsAndSites(1,2);
        guidata(cellrasterGui,guiData);
        cycleSession(cellrasterGui);
