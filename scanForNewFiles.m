...
% expList-----------A structure containing fields for every file detected for the selected mice in the selected daterange
    ...
    %.gavloLog----------Path to the local copy of the galvo log file (generated for inactivation expeirments)
...
includedMice = {'PC005'; 'PC006'; 'PC009'; 'PC010'; 'PC011'; 'PC012'; 'PC013'; 'PC014'; 'PC015'; 'PC016'; 'PC017'};
startedDates = {'', ''};
retiredDates = {...
    'PC005' '2017-07-28'; ...
    'PC006' '2017-07-28'; ...
...
for i = 1:length(newBlocks)
    ...
    if datenum(expDate, 'yyyy-mm-dd') < includedMice{mouseIdx,2} || datenum(expDate, 'yyyy-mm-dd') > includedMice{mouseIdx,3}
        if exist([pathFinder('rawbackup') subject '\' expDate], 'dir'); rmdir([pathFinder('rawbackup') subject '\' expDate], 's'); end
        continue;
    else,  syncfolder([pathFinder('expinfo') subject '\' expDate], [pathFinder('rawbackup') subject '\' expDate], 2);
    end
    ...
    if exist(pathFinder('galvoLog', subject, expDate, sessionNum), 'file')
        expList(end).gavloLog = pathFinder('galvoLog', subject, expDate, sessionNum);
    else
        expList(end).gavloLog = 0;
    end
    ...
    expList(end).blockFunction = str2func([expList(end).expDef '_Blk_Proc']);
...
%% Collect all paths--this also ensures paths are up to date with any changes in pathFinder
for i = 1:length(expList)
    expList(i).rawBlock = pathFinder('rawBlock', expList(i).subject, expList(i).expDate, expList(i).sessionNum); 
    expList(i).rawTimeline = pathFinder('rawTimeline', expList(i).subject, expList(i).expDate, expList(i).sessionNum);
    expList(i).rawFolder = pathFinder('origblockfolder', expList(i).subject, expList(i).expDate, expList(i).sessionNum);
    expList(i).rawParams = pathFinder('rawParameters', expList(i).subject, expList(i).expDate, expList(i).sessionNum);
    expList(i).processedData = pathFinder('processedData', expList(i).subject, expList(i).expDate, expList(i).sessionNum);
    expList(i).sharedData = pathFinder('sharedData', expList(i).subject, expList(i).expDate, expList(i).sessionNum);
    expList(i).suite2POutput = pathFinder('suite2POutput', expList(i).subject, expList(i).expDate, expList(i).sessionNum);
end
