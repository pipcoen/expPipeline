...
for iArea = [23 31 21 setdiff(1:nUniqueAreas,[21 31]) ]
...
%     bw = imerode(bw,strel('disk',5));
    ...
    MyImg(L==1) = iArea; % this modifies the image BUT IT CAN LEAVE DETRITUS OUTSIDE
    k = 1;
    boundary = B{k};
    plot(boundary(:,2), boundary(:,1), 'b', 'LineWidth', 3);
    drawnow
    ThisAcronym = StructureTree.acronym(UniqueAreas(iArea));
    
    stat = regionprops(bw,'centroid');
    for ireg = 1:length(stat)
        text( stat(ireg).Centroid(1), stat(ireg).Centroid(2), [num2str(iArea) ' ' ThisAcronym{1}], 'hori', 'center', 'vert', 'middle' );
    end
