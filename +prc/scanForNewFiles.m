function [expList, currentFileStatus] = scanForNewFiles(rebuildList, checkDirectories, checkExpRigs)
...
%.origTimeline-------Path to the local copy of the timeline file
...
%.origFolder---------Path to the folder were raw data is recorded on zserver
...
elseif ~rebuildList, processList = processList(~contains(processList,{expList.origFolder}'));
end
...
for i = 1:length(processList)
    ...
    if exist(tempLoc.rawBlock, 'file'); load(tempLoc.rawBlock, 'block'); b = block;
        ...
        if exist(tempLoc.origTimeline, 'file'); load(tempLoc.origTimeline);
...
if checkDirectories
    for i = 1:length(expList)
        ...
        txtF = [dir([fileparts(expList(i).origFolder) '\*Exclude*.txt']); dir([expList(i).origFolder '\*Exclude*.txt'])];
...
if checkExpRigs
    ...
    for i = expRigs'
        if strcmp(expList(i).rigName, 'lilrig-stim') && ~isempty(dir([fileparts(expList(i).origFolder) '\*hys*']))
...
processList = cellfun(@fileparts, {expList.origFolder}', 'uni', 0);
