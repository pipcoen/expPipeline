...
whlDecThr = round(median(corrWheelDiff(~idxZ)./visAziDiff(~idxZ))*60);
...
reactBound = num2cell([stimOnsetIdx stimOnsetIdx+1.5*sR],2);
...
threshCrsIdx = cellfun(@(x) max([nan find(abs(rawWheel(x(1):x(2))-rawWheel(x(1)))>whlDecThr*0.3,1)+x(1)]), reactBound);
choiceCrsIdx = cellfun(@(x) max([nan find(abs(rawWheel(x(1):x(2))-rawWheel(x(1)))>whlDecThr,1)+x(1)]), reactBound);
...
sumWin = 51;
velThresh  = sR*(whlDecThr*0.05)/sumWin; 
...
errorIdx = ismember(stimOnsetIdx, movingIdx);
onsetsByTrial(errorIdx) = deal({[nan nan]});
...
firstMoveTimeDir = cell2mat(cellfun(@(x) x(1,:), moveOnsetsTimeDir, 'uni', 0));

[threshMoveTimeDir, decisionMoveTimeDir] = deal(firstMoveTimeDir*nan);
[threshMoveTimeDir, decisionMoveTimeDir] = deal(firstMoveTimeDir*nan);
...
threshDirection  = arrayfun(@(x) max([nan moveOnsetSign(find(moveOnsetIdx<x, 1, 'last'))]), threshCrsIdx);
threshDirection = ((threshDirection==-1)+1).*(abs(threshDirection));
...
threshMoveDirection(~timeOuts) = threshDirection;
%%
