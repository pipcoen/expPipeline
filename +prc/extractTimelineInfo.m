...
name = {t.Timeline.hw.inputs.name};
column = [t.Timeline.hw.inputs.arrayColumn];

t.timebase = t.Timeline.rawDAQTimestamps';
t.photodiode = t.Timeline.rawDAQData(:,column(strcmp(name,'photoDiode')));

numTrials = length(x.validTrials);

b.trialStartTimes = b.events.trialNumTimes(1:numTrials)';
stimOnTimes = b.block.events.stimPeriodOnOffTimes(b.block.events.stimPeriodOnOffValues)';
...
o.TTL_onsets = t.TTL_onsets(1:numTrials);
...
o.expScreenOnTime = b.stimOnTimes + blockTimelineDelay; %Get expected time of stimulus onset
o.expTTLTime = b.TTL_onsets + blockTimelineDelay; %Get expected time of TTL onset
...
for trial = 1:numTrials
    ...
    %appear
    trialIndex = t.trialStartTimes(trial,1) < t.timebase & t.timebase < t.trialStartTimes(trial,1)+mean(diff(t.trialStartTimes));
