function [eLst] = scanForNewFiles_New(rebldAll)
%%
expDirect = '\\zserver.cortexlab.net\Data\expInfo';
syncMatFiles;
if exist('rebldAll', 'var') && rebldAll; eLst = [];
else; load(savePath('eLst'));
end

lastWeek = datestr(floor(now)-7:datenum(floor(now)), 'yyyy-mm-dd');
allFiles = arrayfun(@(x) dirP([eDir sep '*PC*' sep lasW(x,:) sep '**' sep '*block*']), ...
    (1:size(lasW,1))', 'uni', 0);
