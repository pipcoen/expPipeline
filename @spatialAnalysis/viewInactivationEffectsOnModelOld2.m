...
numMice = length(unique(contBlk.tri.subjectRef));
...
for i = 1:(totalLoops)  
    for j = 1:length(scanPlot.gridXY{1}(:))
        ...
        randSplit = rand(contBlk.tot.trials,1)>0.5;
        contCVBlk = prc.filtBlock(contBlk, randSplit);
        contCVBlk = prc.filtBlock(contCVBlk, prc.makeFreqUniform(contCVBlk.tri.subjectRef, 1, [], totalLaserTrials(j)/numMice));
        contGLM = fit.GLMmulti(contCVBlk, 'simpLogSplitVSplitA');
        contGLM.fit;
        ...
        if i <= normEstRepeats
            ...
            subTestBlk = prc.filtBlock(contBlk, ~randSplit);
            includeIdx = prc.makeFreqUniform(subTestBlk.tri.subjectRef, 1, [], totalLaserTrials(j)/numMice);
        end

        tempBlk = prc.filtBlock(subTestBlk, includeIdx);        
        tempGLM = fit.GLMmulti(tempBlk, 'simpLogSplitVSplitA');
        tempGLM.prmInit = contGLM.prmFits;
