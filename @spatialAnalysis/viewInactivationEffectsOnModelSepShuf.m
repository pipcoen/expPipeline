...
for i = 1:normEstRepeats+nShuffles
    ...
    for j = 1:length(gridXY{1}(:))
        ...
        includeIdx = ismember(uniformLasBlk.tri.inactivation.galvoPosition,galvoRef, 'rows');
        subTestBlk = prc.filtBlock(uniformLasBlk, includeIdx);
        subContBlk = uniformContBlk;
        ...
        deltaGLM = fit.GLMmulti(subTestBlk, 'simpLogSplitVSplitA');
