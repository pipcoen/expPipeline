function RTHistogram(behBlksOrig, vis2Use)
...
for i = 1:nMice
    ...
    allRTs{i,1} = nBlk.tri.outcome.reactionTime(nBlk.tri.stim.visContrast~=0.06);
    allRecordedResps{i,1} = nBlk.tri.outcome.responseRecorded(nBlk.tri.stim.visContrast~=0.06);
    allFirstRT{i,1} = nBlk.tri.outcome.timeToFirstMove(nBlk.tri.stim.visContrast~=0.06);
    allCalcResps{i,1} = nBlk.tri.outcome.responseCalc(nBlk.tri.stim.visContrast~=0.06);
...
histogram(axH, cell2mat(allRTs),0:0.01:1.5, 'FaceColor', 'k', 'EdgeColor', 'none', 'FaceAlpha', 1);
xlim([0 1.5]);
...
%%
export_fig('D:\OneDrive\Papers\Coen_2021\FigureParts\Sup1f_RTHistogram', '-pdf', '-painters');
end