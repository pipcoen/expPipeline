...
for i = 1:6
    ...
    hist(deltaParams{1,2}(normEstRepeats+1:end,i),100)
    box off;
    hold on
    plot([contData(1,2) contData(1,2)], ylim, 'r', 'linewidth', 2)
    pVal = 10^(ceil(log10(scanPlot.pVals(1,2))));
    if pVal > 0.01
        sigLev = 'NS';
    else
...
export_fig('D:\Dropbox (Personal)\TalksAndApps\Papers\Coen_2020\FigureParts\3_modelInactivationVis_V2', '-pdf', '-painters');
end